...
def upsert_to_silver_scd1(spark: SparkSession, table_name: str, config: Config, catalog: str = "main", source_db: str = "bronze", target_db: str = "silver"):
    ...
    logger.info(f"Starting Silver SCD1 Upsert for {silver_table_path}...")
    ...
    def upsert_batch(microBatchDF, batchId):
        ...
        if spark.catalog.tableExists(silver_table_path):
            ...
            logger.info(f"Creating initial Silver SCD1 table: {silver_table_path}")
    ...
    logger.info(f"Silver SCD1 upsert for {table_name} complete.")
...
if __name__ == "__main__":
    ...
    upsert_to_silver_scd1(spark, args.table_name, config, args.catalog, args.source_db, args.target_db)
