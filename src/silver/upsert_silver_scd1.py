...
from src.silver.upsert_silver_fact import standardize_data, apply_data_quality_and_quarantine

def upsert_to_silver_scd1(spark: SparkSession, table_name: str, catalog: str = "main", source_db: str = "bronze", target_db: str = "silver"):
    ...
    print(f"Starting Silver SCD1 Upsert for {silver_table_path}...")
    ...
    def upsert_batch(microBatchDF, batchId):
        ...
        if spark.catalog.tableExists(silver_table_path):
            ...
            print(f"Creating initial Silver SCD1 table: {silver_table_path}")
    ...
    print(f"Silver SCD1 upsert for {table_name} complete.")
...
if __name__ == "__main__":
    ...
    spark = SparkSession.builder.appName(f"Silver_Upsert_SCD1_{args.table_name}").getOrCreate()
    upsert_to_silver_scd1(spark, args.table_name, args.catalog, args.source_db, args.target_db)
