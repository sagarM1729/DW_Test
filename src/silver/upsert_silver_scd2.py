...
def upsert_to_silver_scd2(spark: SparkSession, table_name: str, catalog: str = "main", source_db: str = "bronze", target_db: str = "silver"):
    ...
    checkpoint_path = f"/mnt/checkpoints/silver/{table_name}_scd2"

    print(f"Starting Silver SCD2 Upsert for {silver_table_path}...")
    ...
    def upsert_batch(microBatchDF, batchId):
        ...
        if spark.catalog.tableExists(silver_table_path):
            ...
            print(f"Creating initial Silver SCD2 table: {silver_table_path}")
    ...
    print(f"Silver SCD2 upsert for {table_name} complete.")
...
if __name__ == "__main__":
    ...
    upsert_to_silver_scd2(spark, args.table_name, args.catalog, args.source_db, args.target_db)
